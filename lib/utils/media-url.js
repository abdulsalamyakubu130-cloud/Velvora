...
function resolveStoragePublicUrl(rawValue, bucketCandidates, defaultBucket, fallback) {
  ...
  if (isExternalUrl(value)) {
    ...
    return preferHttpsOnSecurePage(normalizedSupabaseUrl || value)
  }
...
function collectListingImageCandidates(rawValue) {
  if (!rawValue) return []

  if (Array.isArray(rawValue)) {
    return rawValue.flatMap((value) => collectListingImageCandidates(value))
  }

  if (typeof rawValue === 'object') {
    if (Array.isArray(rawValue.images)) return collectListingImageCandidates(rawValue.images)
    if (rawValue.image_url) return collectListingImageCandidates(rawValue.image_url)
    if (rawValue.url) return collectListingImageCandidates(rawValue.url)
    return []
  }
  ...
  if (value.startsWith('[') || value.startsWith('{')) {
    try {
      ...
      return collectListingImageCandidates(parsedValue)
    } catch {
...
export function extractListingImageUrls(rawValue, fallback = []) {
  const seenUrls = new Set()
  const resolvedUrls = []

  for (const candidate of collectListingImageCandidates(rawValue)) {
    const resolvedUrl = resolveListingImageUrl(candidate, '')
    if (!resolvedUrl || seenUrls.has(resolvedUrl)) continue
    seenUrls.add(resolvedUrl)
    resolvedUrls.push(resolvedUrl)
  }

  return resolvedUrls.length ? resolvedUrls : fallback
}
